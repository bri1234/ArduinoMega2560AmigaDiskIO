...
#define PIN_WRITE_DATA_MASK (1 << 5) // The mask used to set this pin high or low

// PIN 4 - READ DATA
#define PIN_READ_DATA 4             // Reads RAW floppy data on this pin.
#define PIN_READ_DATA_MASK (1 << 5) // The mask for the port
#define PIN_READ_DATA_PORT PING     // The port the above pin is on
#define PIN_READ_DATA_PORT_WRITE PORTG
#define PIN_READ_DATA_IO_DIR DDRG
...
#define PIN_WRITE_GATE 15            // This pin enables writing to the disk
#define PIN_WRITE_GATE_PORT PORTJ    // The actual port the above pin is on
#define PIN_WRITE_GATE_MASK (1 << 0) // The port pin mask for the gate

// PIN A1 - CHECK WRITE PROTECTION
#define PIN_WRITE_PROTECTED 16 // To check if the disk is write protected

// PIN A2 - CTS Pin from UART
#define PIN_CTS A1            // Pin linked to the CTS pin
#define PIN_CTS_PORT PORTF    // Port the CTS pin is on
#define PIN_CTS_MASK (1 << 1) // Binary mask to control it with
...
#define DISABLE_DENSITY_DETECTION 0
#define SLOWER_DISK_SEEKING 1
#define ALWAYS_INDEX_ALIGN_WRITES 0
...
void smalldelay(unsigned long delayTime)
{
  ...
  for (unsigned long i = 0; i < delayTime; i++)
  {
    ...
    while (TCNT0 < 250)
    {
    }; // wait 1ms, we could do this more accuratly, but im not bothered
  }
  TCCR0B = 0; // turn off
}
...
void smalldelay100us(unsigned long delayTime)
{
  ...
  for (unsigned long i = 0; i < delayTime; i++)
  {
    ...
    while (TCNT0 < 200)
    {
    }; // wait 100us, we could do this more accuratly, but im not bothered
  }
  TCCR0B = 0; // turn off
}
...
void runDiagnostic()
{
  ...
  case '4': // Data pulse test (with timeout)
  {
    ...
    for (unsigned int b = 0; b < 20; b++)
    {
      for (unsigned int a = 0; a < 60000; a++)
      {
        if (PIN_READ_DATA_PORT & PIN_READ_DATA_MASK)
          state1 = true;
        else
          state2 = true;
        if (state1 && state2)
...
void handleNoClickSeek()
{
  ...
  if (digitalRead(PIN_DISK_CHANGE) == HIGH)
    writeByteToUART('1');
  else
    writeByteToUART('#');
...
bool gotoTrackX(bool reportDiskChange)
{
  ...
  // Validate
  if ((track1 < '0') || (track1 > '9'))
    return false;
  if ((track2 < '0') || (track2 > '9'))
    return false;
  ...
  if (track < 0)
    return false;
  if (track > 83)
    return false; // yes amiga could read track 83!
...
void writePrecompTrack()
{
  ...
  if (waitForIndex || ALWAYS_INDEX_ALIGN_WRITES)
  ...
  do
  {
    ...
    if (currentByte & 0x04)
      pulseStart -= 2; // Pulse should be early, so just move the pulse start back
    if (currentByte & 0x08)
      pulseStart += 2; // Pulse should be late, so move the pulse start forward
...
void writeFluxTrack()
{
  ...
  if (!terminateAtIndex)
    EICRA = 0; // disable if we're not using terminate at index
...
void measureCurrentDisk()
{
  ...
  sendInt(TIMING_DD_MIDDLE_4us + offsetIgnore);
  sendString(", ");
  sendInt(TIMING_DD_MIDDLE_6us + offsetIgnore);
  sendString(", ");
  sendInt(TIMING_DD_MIDDLE_8us + offsetIgnore);
  sendString("\n");
  sendString("Ticks for Upper bound for 4, 6 and 8 uS: ");
  sendInt(TIMING_DD_UPPER_4us + offsetIgnore);
  sendString(", ");
  sendInt(TIMING_DD_UPPER_6us + offsetIgnore);
  sendString(", ");
  sendInt(TIMING_DD_UPPER_8us + offsetIgnore);
  sendString("\n");
  sendString("Ticks for HD Upper bound for 2, 3 uS: ");
  sendInt(TIMING_HD_UPPER_2us + offsetIgnore);
  sendString(", ");
  sendInt(TIMING_HD_UPPER_3us + offsetIgnore);
  sendString(", ");
  sendString("\n");
  ...
  for (int a = 1; a <= 255; a++)
  {
    if (cc[a] > 1)
    {
      ...
      else if (a < TIMING_HD_UPPER_3us + offsetIgnore)
      {
        sendString("3us (HD)");
      }
      else if (a < TIMING_DD_UPPER_4us + offsetIgnore)
      {
        sendString("4us (DD/HD)");
      }
      else if (a < TIMING_DD_UPPER_6us + offsetIgnore)
      {
        sendString("6us (DD)");
      }
      else if (a < TIMING_DD_UPPER_8us + offsetIgnore)
      {
        sendString("8us (DD)");
      }
      else
  ...
  while (totalBits < target)
  {
    for (register unsigned char bits = 0; bits < 4; bits++)
    {
      ...
      if (counter < TIMING_DD_UPPER_4us)
        DataOutputByte |= B00000001, totalBits += 2;
      else // this accounts for just a '1' or a '01' as two '1' arent allowed in a row
        if (counter < TIMING_DD_UPPER_6us)
          DataOutputByte |= B00000010, totalBits += 3;
        else if (counter < TIMING_DD_UPPER_8us)
          DataOutputByte |= B00000011, totalBits += 4;
        else
  ...
  {                                                                                   \
    ...
    {                                                                                 \
      ...
      if (counter >= TIMING_DD_UPPER_2us - HALF_TIMING_OVERHEAD)                      \
        counter -= TIMING_DD_UPPER_2us - HALF_TIMING_OVERHEAD;                        \
      else                                                                            \
        counter = 0;                                                                  \
    }                                                                                 \
    else if (counter < TIMING_DD_UPPER_6us - HALF_TIMING_OVERHEAD)                    \
    {                                                                                 \
      DataOutputByte |= p6uSec;                                                       \
      counter -= TIMING_DD_UPPER_4us - HALF_TIMING_OVERHEAD;                          \
    }                                                                                 \
    else if (counter < TIMING_DD_UPPER_8us - HALF_TIMING_OVERHEAD)                    \
    {                                                                                 \
  ...
  {                                                                            \
    ...
    {                                                                          \
      ...
      if (counter >= TIMING_DD_UPPER_2us - TIMING_OVERHEAD)                    \
        counter -= TIMING_DD_UPPER_2us - TIMING_OVERHEAD;                      \
      else                                                                     \
        counter = 0;                                                           \
    }                                                                          \
    else if (counter < TIMING_DD_UPPER_6us - TIMING_OVERHEAD)                  \
    {                                                                          \
      DataOutputByte |= p6uSec;                                                \
      counter -= TIMING_DD_UPPER_4us - TIMING_OVERHEAD;                        \
    }                                                                          \
    else if (counter < TIMING_DD_UPPER_8us - TIMING_OVERHEAD)                  \
    {                                                                          \
      DataOutputByte |= p8uSec;                                                \
...
void readContinuousStream(bool highPrecision)
{
  ...
  {
    if (TCNT0 > 250)
      break; // this is to stop the inteface freezing if theres no disk in the drive
  };
  TCNT0 = 0; // reset
...
{ // 4 to start an interrupt, 4 ish to jump to interrupt
  __asm__ __volatile__(
      ...
      "m"(PCICR), "m"(TCNT2), "i" _SFR_IO_ADDR(TIFR2), "i"(OCF2B) :);
}
...
void readContinuousStreamPLL_ASM()
{
  ...
  {
    if (TCNT0 > 250)
      break; // this is to stop the interface freezing if theres no disk in the drive
  };
  TCNT0 = 0; // reset
  while (!(PIN_READ_DATA_PORT & PIN_READ_DATA_MASK))
  {
  ...
  __asm__ volatile(
      ...
      WAIT_FLOR_FLUX_CLASSIC("C1")
          READ_UNROLLED_LOOP_PLL("0x40", "0x80", "0xC0", "C1") "mov r24, r21             \n" // average = counter
                                                               "mov r29, r27             \n" // average = counter
      ...
      HANDLE_INDEX_DETECTION("CLASSIC")
          ...
          WAIT_FLOR_FLUX_CLASSIC("C2")
              READ_UNROLLED_LOOP_PLL("0x10", "0x20", "0x30", "C2") "add r24, r21             \n" // average += counter
                                                                   "add r29, r27             \n" // average += counter
                                                                   "sts %4, r25              \n" // UDR0 = toTransmit
      ...
      WAIT_FLOR_FLUX_CLASSIC("C3")
          READ_UNROLLED_LOOP_PLL("0x04", "0x08", "0x0C", "C3") "add r24, r21             \n" // average += counter
                                                               "add r29, r27             \n" // average += counter
      ...
      WAIT_FLOR_FLUX_CLASSIC("C4")
          READ_UNROLLED_LOOP_PLL("0x01", "0x02", "0x03", "C4") "add r24, r21             \n" // average += counter
                                                               "add r29, r27             \n" // average += counter
                                                               "mov r25, r24             \n" // toTransmit = average
                                                               ...
                                                               "pop r18                 \n" // Store r18
                                                               "pop r17                 \n" // Store r17
      ::/*0*/ "i" _SFR_IO_ADDR(TIFR2),
      /*1*/ "i" _SFR_IO_ADDR(PCIFR), /*2*/ "i"(PCIF2), /*3*/ "i" _SFR_IO_ADDR(TCNT0), /*4*/ "m"(UDR0),
...
void fluxStreamer()
{
  ...
  __asm__ __volatile__(
      ...
      "sub r20, r22                    \n"
      "add r22, r20                    \n" CHECK_RECEIVED_FLUX("standard") "sbis  %12, %13                 \n" // Check the READ DATA pin
                                                                           "rjmp  .-4                      \n" // 2 Jump back if its still within a pulse
                                                                           "sbi %14, %15                   \n" // 2 Clear the flag again incase it was singalled by the wrong change
                                                                           "sei                            \n" // Re-enable interrupts
      ...
      "pop r21                          \n"
      "pop r20                          \n" ::"i" _SFR_IO_ADDR(EIFR),
      "i"(INTF0), "i"(MAX_FLUX_REPEAT), "m"(UDR0), "m"(UCSR0A), "i"(RXC0), "i" _SFR_IO_ADDR(TCNT0), "i"(FLUX_REPEAT_OFFSET),
...
#define READ_UNROLLED_LOOP_HD_ASM_BITMODE_DRAWBRIDGE_PLUS(id)                                                           \
  asm("sbic  %0, %1                     \n" /* fetch TIFR1 & bit(ICF1) and skip the next line if its clear */           \
      ...
      "sbi %2, %3                      \n" /* Reset overflow flag */                                                    \
      "out %7, r26                     \n" /* 1 Tick,   Reset Timer0 value */                                           \
      ::"i" _SFR_IO_ADDR(TIFR1),                                                                                        \
      "i"(ICF1), "i" _SFR_IO_ADDR(TIFR0), "i"(OCF0B), "m"(ICR1L), "i" _SFR_IO_ADDR(EIFR), "i"(INTF0), "i" _SFR_IO_ADDR(TCNT0) :);
...
ISR(WDT_vect, ISR_NAKED)
{
  asm("ldi r25, 0xFF          \n" // Set timed out register
      ...
      "sbi %1, %2            \n"  // Pullup
      "reti                  \n"  // And return
      ::"i" _SFR_IO_ADDR(PIN_READ_DATA_IO_DIR),
      "i" _SFR_IO_ADDR(PIN_READ_DATA_PORT_WRITE), "i"(PIN_READ_DATA) :); // PIN_READ_DATA isnt strictly correct
}
...
void testDiskDensity()
{
  ...
  if ((DISABLE_DENSITY_DETECTION) || (digitalRead(PIN_HD) == LOW))
  {
    writeByteToUART('D');
    return;
  }
  ...
  while (overflows < MAX_OVERFLOWS)
  {
    ...
    {
      if ((counter > 8) && (counter <= TIMING_HD_UPPER_3us))
        HDPulses++;
      else if ((counter > TIMING_DD_UPPER_4us + 8) && (counter <= TIMING_DD_UPPER_8us))
        DDPulses++;
      else
        errors++;
      if ((HDPulses > 6000) || (DDPulses > 6000) || (errors > 18000))
        break;
    }
  }
  ...
  if ((HDPulses < 10) && (DDPulses < 10))
    writeByteToUART('x');
  else // no disk
    if (HDPulses > DDPulses)
      writeByteToUART('H');
    else
      writeByteToUART('D');
...
void loop()
{
  ...
  {
  case 'x':
    break; // this is ignored.  It's to help 'stop streaming' mode if it gets stuck on startup
  case 'R':
    doReset();
    break;
  case 'M':
    if (!driveEnabled)
      sendString("Drive motor not switched on.\n");
    else
      measureCurrentDisk();
    break;
  case 'P':
  ...
  case 'T':
    ...
    break;
  case 't':
    if (DISABLE_DENSITY_DETECTION)
      writeByteToUART('D');
    else if (digitalRead(PIN_HD) == LOW)
      writeByteToUART('D');
    else
      writeByteToUART('H');
    break;
  ...
  case '?':
    writeByteToUART('1'); // Success
    writeByteToUART('V'); // Followed
    writeByteToUART('1'); // By
    writeByteToUART(','); // Advanced controller version
  ...
  case '@':
    ...
    writeByteToUART(FLAGS_HIGH_PRECISION_SUPPORT | FLAGS_FLUX_READ |
                    ...
                    (FLAGS_DISKCHANGE_SUPPORT) |
                    (DISABLE_DENSITY_DETECTION ? 0 : FLAGS_DENSITYDETECT_ENABLED) |
                    (SLOWER_DISK_SEEKING ? FLAGS_SLOWSEEKING_MODE : 0) |
                    (ALWAYS_INDEX_ALIGN_WRITES ? FLAGS_INDEX_ALIGN_MODE : 0));
